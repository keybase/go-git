...
func reachableObjects(
	...
	pendingDepths := make(map[plumbing.Hash]int)
	addPendingParents(pendingDepths, commit, 0)
	...
	for {
		...
		currDepth, ok := pendingDepths[commit.Hash]
		if ok {
			delete(pendingDepths, commit.Hash)
		}

		if ignore != nil || currDepth <= 5 {
			addPendingParents(pendingDepths, commit, currDepth)
		}
...
func addPendingParents(pendingDepths map[plumbing.Hash]int, commit *object.Commit, currDepth int) {
	for _, p := range commit.ParentHashes {
		pendingDepths[p] = currDepth + 1
	}
}
